...
def estimate_pq_break_from_bounce(calib: dict, model: str = "wall", power: float = 1.0,
    ...
    if model == "bounce":
        ...
        S_break = S_E * w_over_R
    elif model == "patch":
        S_break = S_E * (w_over_R**2)
    elif model == "power":
        S_break = S_E * (w_over_R**float(power))
    else:
...
def main():
    ...
    ap.add_argument("--pq_break_S_model", default="wall", choices=["bounce","wall","patch","power"],
    ...
    if args.calib:
        ...
        if fa:
            ...
            if pq_proxy is not None and pq_proxy.get("ok"):
                print("\n--- PQ-breaking proxy from bounce (heuristic) ---")
